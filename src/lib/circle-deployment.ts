...
export class CircleAccountDeployment {
  ...
  async deployAccount() {
    ...
    try {
      ...
      const bundlerClient = createBundlerClient({
        ...
        transport: http(
          `https://api.pimlico.io/v2/84532/rpc?apikey=pim_LiPzgUW8Y9rAntyZLntsjR`
        ),
      });
      ...
      console.log("Account deployment initiated. UserOperation hash:", hash);

      const receipt = await bundlerClient.waitForUserOperationReceipt({ hash });
      console.log(
        "Account deployed. Transaction hash:",
  ...
  async sendTransaction(to: string, value: bigint, data: string = "0x") {
    if (!this.account) {
      throw new Error(
        "Account not initialized. Call initializeAccount() first."
      );
...
export function validateChainId(
  ...
  const chainList = isTestnet ? chains.testnet : chains.mainnet;
  return chainId in chainList;
}
